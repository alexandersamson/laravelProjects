<h5>Assignable lead investigators</h5>
@if(count($data['investigators']) > 0)
    @foreach($data['investigators'] as $investigator)
            <div class="d-flex flex-row align-items-center border">
                ...
                <div class="pr-1">
                    <a class="btn text-sm-left align-self-sm-start btn-sm btn-primary font-weight-light selectBtnRadio" data-url="addLeadInvestigator" data-value="{{$investigator->id}}" id="btnRadio{{$investigator->id}}" href="#}">Select this user</a>
                </div>
            </div>
    @endforeach
